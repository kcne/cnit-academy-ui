...
export default function RoleRequestForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (justification.length < 20) {
      setError("Justification must be longer than 20 characters");
      return;
    }
    api.post("/api/role-request", {
  ...
  return (
    <div className="max-w-xl mx-auto mt-12 p-6 border rounded-md shadow">
      ...
      <p className="mb-6 text-gray-600">
        Request additional permissions or roles for your account
      </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-4">
          ...
          <p>
            Role requests are reviewed by administrators. Please provide a
            detailed justification for your request.
          </p>
        </div>

        <label className="block font-medium mb-2" htmlFor="role">
          Role
        </label>
        <select
          id="role"
        ...
        <label className="block font-medium mb-3" htmlFor="justification">
          Justification
        </label>
        <textarea
          id="justification"
